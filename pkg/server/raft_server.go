...
func (s *RaftServer) showMetrics() {
	...
	recvcData := make([]opts.LineData, 0)
	sendcData := make([]opts.LineData, 0)
	propcData := make([]opts.LineData, 0)
	...
	render := func() *components.Page {
		...
		chLegend := []string{"Recvc", "Sendc", "Propc"}
		...
		chLine.SetGlobalOptions(
			charts.WithInitializationOpts(opts.Initialization{Theme: types.ThemeWesteros}),
			charts.WithTitleOpts(opts.Title{Title: fmt.Sprintf("%s通道", name)}),
			charts.WithLegendOpts(opts.Legend{Show: true, Data: chLegend}),
		)

		chLine.SetXAxis(xAxis).
			AddSeries("Recvc", recvcData).
			AddSeries("Sendc", sendcData).
			AddSeries("Propc", propcData).
			SetSeriesOptions(charts.WithLineChartOpts(opts.LineChart{Smooth: true}))
		...
		page.AddCharts(srLine, chLine, nodeLine, grLine)
	...
	go func() {
		...
		fd, err := os.OpenFile(path.Join(s.dir, "raft_"+strconv.FormatUint(s.id, 10), "metric.html"), os.O_WRONLY|os.O_CREATE, 0644)
		if err != nil {
		...
		for {
			...
			if !first {
				...
				recvcData = append(recvcData, opts.LineData{Value: status.RecvcSize})
				sendcData = append(sendcData, opts.LineData{Value: status.SendcSize})
				propcData = append(propcData, opts.LineData{Value: status.PropcSize})
				...
				if size > 180 {
					...
					recvcData = recvcData[start:]
					sendcData = sendcData[start:]
					propcData = propcData[start:]
